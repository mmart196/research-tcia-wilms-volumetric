...
**Results:** Of {F_META_ANY} subjects with any kidney tumor segmentation, {F_PAIRED} had computable CT-referenced volumes at both timepoints ({C_0534}/{N} = {BILAT_SHARE}% from the bilateral-disease trial AREN0534). Median percent volume change at the first post-chemotherapy timepoint was {S0532_MED}% in AREN0532 (delayed-nephrectomy subset, n = {S0532_N}), {S0533_MED}% in AREN0533 (stage III–IV, n = {S0533_N}), and {S0534_MED}% in AREN0534 (bilateral, n = {S0534_N}); because response intervals differ across protocols and are not recoverable, these profiles are not directly comparable across trials. Pooled across trials, median volume fell from {PRE_MED} mL to {POST_MED} mL (median change {PCT_MED}%; Hodges–Lehmann estimate {HL_EST}%, 95% CI {HL_LO}% to {HL_HI}%; Wilcoxon p = {W_P}), decreasing in {N_DEC} of {N} subjects.
...
**Figure 1:** Waterfall plot of percent tumor volume change from Pre-dose to first post-chemotherapy timepoint (n = {N}).
...
Across three Children's Oncology Group trials, neoadjuvant chemotherapy reduced Wilms tumor volume by a median of {S0532_MED}%, {S0533_MED}%, and {S0534_MED}% in the delayed-nephrectomy, advanced-stage, and bilateral cohorts respectively — three protocol-defined profiles that are not directly comparable because their response intervals differ. Decreases occurred in {N_DEC} of {N} children. Publicly released expert 3D segmentations enable fully reproducible, contour-based quantification of treatment response at cohort scale. The benchmark corpus and pipeline are the durable contribution: volumetric response profiles are a reference for response-adapted trial design and a baseline for the clinical-covariate linkage this dataset makes possible.

## Data Availability

Public annotation data: The Cancer Imaging Archive, collections AREN0532, AREN0533, AREN0534 [1-9]. Analysis code, cohort manifest, and the rendering pipeline that produced every number in this manuscript: project repository (URL to be added at submission; currently private during review).
